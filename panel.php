<?php
	...
	if(isset($_POST["btnCrearWeb"])){
		if($_POST["dominio"] == ""){
			$msg = "¡Tiene que igresar un nombre!";
		}else{
			...
			while($fila = mysqli_fetch_array($response,MYSQLI_ASSOC)){
				if ($fila["dominio"] == $dominio){
					$msg = "¡Ya existe este dominio!";
					$mostrar = false;
				}
			}
			if($mostrar){
				$date = date("Y-m-d");
				$sql = 'INSERT INTO `webs`(`idWeb`, `idUsuario`, `dominio`, `fechaCreacion`) VALUES (null,"'.$_SESSION["idUsuario"].'","'.$dominio.'","'.$date.'")';
	...
	while($fila = mysqli_fetch_array($response,MYSQLI_ASSOC)){
		if ($fila["idUsuario"] == $_SESSION["idUsuario"]){
			$lista.= '<tr>
						<td><a href="../'.$fila["dominio"].'">'.$fila["dominio"].'</a></td>
						<td><a href="descargar.php?zip='.$fila["dominio"].'"><i class="fa-solid fa-download small" style="color: #004d40"></i></a></td>
						<td><a href="eliminar.php?dominio='.$fila["dominio"].'"><i class="fa-solid fa-trash small"style="color: #004d40"></i></a></td>
					</tr>';
		}
	}
	$lista.= "</table>"
...
<body>
	<nav>
	    <div class="nav-wrapper  teal darken-4">
	      	<a href="#" class="brand-logo center">¡Bienvenido a tu panel!</a>
	    </div>
  	</nav>
  	<br>
  	<br>
  	...
  	<content >
	  		...
	  		<di class="row ">
		      	<div class="card col s10 m6 l6 offset-s1 offset-m3 offset-l3 hoverable ee">
		        	<div class="flow-text center">
	            		<p><b>Cerrar sesión de <?php echo $_SESSION['idUsuario'];?></b></p> 
	        		</div>
		      	</div>
	      	</di>
      	</a>
  		...
  		<div class="row">
	      	<div class="card col s10 m6 l6 offset-s1 offset-m3 offset-l3">
            	<br>
            	<div class="flow-text center" >
            		<p><b>Generar Web de:</b></p> 
        		</div>
				<form  method="POST" id="formulario">
					<div class="row">
				        <div class="input-field col s10 m10 l10 offset-s1 offset-m1 offset-l1">
				          	<textarea id="textarea1" class="materialize-textarea" name="dominio"></textarea>
				          	 <label for="textarea1">Nombre:</label>
				        </div>
				    </div>
				    <div class="row">
				    	<div class="col offset-s3 offset-m4 offset-l5">
				    		<input type="submit" class="center-align waves-effect waves-light btn" id="btnCrearWeb" name="btnCrearWeb" value="Crear Web">
				    	</div>
				    </div>
				    <div class="msg">
				    	<?php
